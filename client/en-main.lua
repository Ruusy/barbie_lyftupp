...
function OpenActionMenuInteraction(target)
    ...
    function(data, menu)
		...
		if data.current.value == 'drag' then			
			...
			ESX.ShowNotification('Kimi Kucaklıycanki ?')
			...
			--if hasRope == true then
				...
				if distance ~= -1 and distance <= 3.0 then
					...
					ESX.ShowNotification("Yakinlarda kimse yok...")
				end
			else
...
Citizen.CreateThread(function()
  while true do
    Citizen.Wait(0)
    if IsControlJustReleased(0, Keys['PAGEUP']) and not ESX.UI.Menu.IsOpen('default', GetCurrentResourceName(), 'action_menu') then
